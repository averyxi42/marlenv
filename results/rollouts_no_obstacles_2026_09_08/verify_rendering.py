...
checks=[]
for meta_path in sorted((ROOT/'diagrams').glob('rollout_no_obstacles_*.json')):
...
for budget in (40,):
    with Image.open(ROOT/'diagrams'/f'rollout_no_obstacles_trio{budget}_b12_dp1_seed0.gif') as combined:
        for column,arm in enumerate(['solo','ego','ceiling']):
            with Image.open(ROOT/'diagrams'/f'rollout_no_obstacles_{arm}{budget}_b12_dp1_seed0.gif') as source:
        ...
        checks.append(dict(gif=f'rollout_no_obstacles_trio{budget}_b12_dp1_seed0.gif',
