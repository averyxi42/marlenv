...
labels=['solo40','ego40','ceiling40']
headings=np.array([[-1,0],[0,1],[1,0],[0,-1]])
reference=None
checks=[]
for label in labels:
    stem=ROOT/'diagrams'/f'rollout_no_obstacles_{label}_b12_dp1_seed0'
    meta=json.loads(stem.with_suffix('.json').read_text())
    prior=json.loads((ROOT/'diagrams'/f'rollout_corrected_{label}_b12_dp1_seed0.json').read_text())
    assert meta['settings']['obstacle_density']==0
    for key,value in prior['settings'].items():
        if key not in ('obstacle_density','out'):
            assert meta['settings'][key]==value,(label,key,'unintended setting change')
    assert meta['model_sha256']==prior['model_sha256']
    assert meta['policy_sha256']==prior['policy_sha256']
...
print(json.dumps(dict(checks=checks, verified=['Only obstacle density and output paths differ from the prior 40k runs; checkpoint and policy hashes match',
    'All three actual prefixes and initial GIF frames identical',
